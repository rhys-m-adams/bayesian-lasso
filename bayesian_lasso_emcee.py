...
def bayesian_lasso(A,y, penalty, start, burn_in=0, num_iterations=0, nwalkers=250, autocorr_scan = 2000):
    ...
    for ac in autocorr:
        ...
        if ind[0].shape[0]:
            ...
            ind = ac.shape[0]
        if ind < 2:
            sample_rate.append(1)
        else:
            myf = np.linalg.lstsq(np.array([list(range(1,ind+1))]).T, np.log(ac[:ind]))[0]
            if not (np.isfinite(np.ceil(-np.log(10)/myf))):
                print('warning: choosing maximum autocorrelation length, change autocorr_scan parameter to modify sampling rate')
                sample_rate.append(autocorr_scan)
            else:
